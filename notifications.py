...
class NotificationDispatcher:
    ...
    def send_twilio_sms(self, city, event, text, targets=None):
        sid = os.getenv("TWILIO_ACCOUNT_SID", "").strip()
        token = os.getenv("TWILIO_AUTH_TOKEN", "").strip()
        sender = os.getenv("TWILIO_FROM_NUMBER", "").strip()
        env_targets = [x.strip() for x in os.getenv("TWILIO_TO_NUMBERS", "").split(",") if x.strip()]
        recipients = [x.strip() for x in (targets or []) if str(x).strip()] if targets is not None else env_targets

        if not (sid and token and sender):
            return [self._status("twilio", "SKIPPED", city, event, "twilio_not_configured")]
        if not recipients:
            return [self._status("twilio", "SKIPPED", city, event, "twilio_no_targets")]
    ...
    def send_alert(self, city, event, text, targets=None):
        subject = f"Farmer Alert: {event.upper()} in {city}"
        rows = []
        rows.extend(self.send_twilio_sms(city, event, text, targets=targets))
