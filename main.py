...
from region_contacts import RegionContactStore
...
class Dashboard(tk.Frame):
    def __init__(self, master, username, role, on_logout):
        ...
        self._contacts   = RegionContactStore(regions=self._loc.keys())
    ...
    def _can(self, action):
        allowed = {
            "admin": {"refresh", "auto_send", "send_selected", "send_all", "export", "incident_update", "contact_manage"},
            "analyst": {"refresh", "auto_send", "send_selected", "export", "incident_update", "contact_manage"},
    ...
    def _build_header(self):
        ...
        # contacts manager
        cm = tk.Button(right, text="CONTACTS",
                       font=("Courier New", 9, "bold"),
                       bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                       padx=10, pady=4, cursor="hand2",
                       activebackground=C["accent2"],
                       activeforeground="#000",
                       command=self._open_contacts_manager)
        cm.pack(side="left", padx=(0, 10))
    ...
    def _open_contacts_manager(self):
        if hasattr(self, "_cm_window") and self._cm_window and self._cm_window.winfo_exists():
            self._cm_window.lift()
            self._cm_window.focus_force()
            return

        win = tk.Toplevel(self)
        win.title("Regional Contact Manager")
        win.geometry("1180x760")
        win.minsize(980, 620)
        win.configure(bg=C["bg"])
        self._cm_window = win

        top = tk.Frame(win, bg=C["bg2"], pady=10, padx=12)
        top.pack(fill="x")
        tk.Label(top, text="REGIONAL CONTACT MANAGER",
                 font=("Courier New", 14, "bold"),
                 bg=C["bg2"], fg=C["accent2"]).pack(side="left")
        tk.Button(top, text="Use Selected Region", font=("Courier New", 9, "bold"),
                  bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                  cursor="hand2", command=self._cm_sync_selected_region).pack(side="right", padx=(8, 0))
        tk.Button(top, text="Close", font=("Courier New", 9, "bold"),
                  bg=C["danger"], fg="#fff", relief="flat", bd=0,
        ...
        # Left: regions with counts
        left = tk.Frame(body, bg=C["bg2"], padx=8, pady=8,
                        highlightthickness=1, highlightbackground=C["border"])
        left.grid(row=0, column=0, sticky="nsw")
        tk.Label(left, text="Regions", bg=C["bg2"], fg=C["muted"],
                 font=("Courier New", 9, "bold")).pack(anchor="w", pady=(0, 6))
        self._cm_region_filter = tk.StringVar(value="")
        tk.Entry(left, textvariable=self._cm_region_filter, width=24,
                 bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                 insertbackground=C["text"]).pack(fill="x", ipady=3, pady=(0, 8))

        self._cm_region_list = tk.Listbox(left, width=28, height=26,
                                          bg=C["bg3"], fg=C["text"],
                                          selectbackground=C["selected"],
                                          selectforeground=C["text"],
                                          relief="flat", bd=0)
        self._cm_region_list.pack(fill="both", expand=True)
        self._cm_region_list.bind("<<ListboxSelect>>", lambda _e: self._cm_refresh_contacts())
        self._cm_region_filter.trace_add("write", lambda *_: self._cm_refresh_region_list())

        # Right: contacts and tools
        right = tk.Frame(body, bg=C["bg2"], padx=10, pady=10,
                         highlightthickness=1, highlightbackground=C["border"])
        right.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        right.columnconfigure(0, weight=1)
        right.rowconfigure(2, weight=1)

        hdr = tk.Frame(right, bg=C["bg2"])
        hdr.grid(row=0, column=0, sticky="ew")
        self._cm_selected_label = tk.StringVar(value="Region: -")
        tk.Label(hdr, textvariable=self._cm_selected_label,
                 bg=C["bg2"], fg=C["accent"], font=("Courier New", 10, "bold")).pack(side="left")
        tk.Label(hdr, text="Search", bg=C["bg2"], fg=C["muted"],
                 font=("Courier New", 8)).pack(side="right")
        self._cm_search = tk.StringVar(value="")
        sr = tk.Entry(hdr, textvariable=self._cm_search, width=28,
                      bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                      insertbackground=C["text"])
        sr.pack(side="right", padx=(0, 6), ipady=3)
        self._cm_search.trace_add("write", lambda *_: self._cm_refresh_contacts())

        tools = tk.Frame(right, bg=C["bg2"])
        tools.grid(row=1, column=0, sticky="ew", pady=(8, 6))
        for txt, bg, fg, cmd in [
            ("Import CSV", C["bg3"], C["text"], self._cm_import_region_csv),
            ("Export CSV", C["accent"], "#000", self._cm_export_region_csv),
            ("Send Test Alert", C["warn"], "#000", self._cm_send_test_alert),
            ("Refresh", C["bg3"], C["text"], self._cm_refresh_contacts),
        ]:
            tk.Button(tools, text=txt, font=("Courier New", 8, "bold"),
                      bg=bg, fg=fg, relief="flat", bd=0,
                      cursor="hand2", command=cmd).pack(side="right", padx=3)

        cols = ("id", "name", "number", "updated")
        self._cm_tree = ttk.Treeview(right, columns=cols, show="headings")
        self._cm_tree.heading("id", text="ID")
        self._cm_tree.heading("name", text="Name")
        self._cm_tree.heading("number", text="Phone Number")
        self._cm_tree.heading("updated", text="Updated")
        self._cm_tree.column("id", width=80, anchor="center")
        self._cm_tree.column("name", width=200)
        self._cm_tree.column("number", width=190)
        self._cm_tree.column("updated", width=160)
        self._cm_tree.grid(row=2, column=0, sticky="nsew")
        self._cm_tree.bind("<<TreeviewSelect>>", lambda _e: self._cm_on_contact_select())

        form = tk.Frame(right, bg=C["bg2"])
        form.grid(row=3, column=0, sticky="ew", pady=(8, 0))
        tk.Label(form, text="Name", bg=C["bg2"], fg=C["muted"], font=("Courier New", 8)).pack(side="left")
        self._cm_name = tk.StringVar(value="")
        tk.Entry(form, textvariable=self._cm_name, width=20,
                 bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                 insertbackground=C["text"]).pack(side="left", padx=4, ipady=3)
        tk.Label(form, text="Number", bg=C["bg2"], fg=C["muted"], font=("Courier New", 8)).pack(side="left")
        self._cm_number = tk.StringVar(value="")
        tk.Entry(form, textvariable=self._cm_number, width=20,
                 bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                 insertbackground=C["text"]).pack(side="left", padx=4, ipady=3)

        self._cm_btn_add = tk.Button(form, text="Add", font=("Courier New", 8, "bold"),
                                     bg=C["ok"], fg="#000", relief="flat", bd=0,
                                     cursor="hand2", command=self._cm_add)
        self._cm_btn_add.pack(side="right", padx=3)
        self._cm_btn_update = tk.Button(form, text="Update", font=("Courier New", 8, "bold"),
                                        bg=C["warn"], fg="#000", relief="flat", bd=0,
                                        cursor="hand2", command=self._cm_update)
        self._cm_btn_update.pack(side="right", padx=3)
        self._cm_btn_delete = tk.Button(form, text="Delete", font=("Courier New", 8, "bold"),
                                        bg=C["danger"], fg="#fff", relief="flat", bd=0,
                                        cursor="hand2", command=self._cm_delete)
        self._cm_btn_delete.pack(side="right", padx=3)
        tk.Button(form, text="Clear", font=("Courier New", 8, "bold"),
                  bg=C["bg3"], fg=C["text"], relief="flat", bd=0,
                  cursor="hand2", command=self._cm_clear_form).pack(side="right", padx=3)

        bulk = tk.Frame(right, bg=C["bg2"])
        bulk.grid(row=4, column=0, sticky="ew", pady=(8, 0))
        tk.Label(bulk, text="Bulk Add (one per line: Name,+123... or +123...)",
                 bg=C["bg2"], fg=C["muted"], font=("Courier New", 8)).pack(anchor="w")
        self._cm_bulk_text = tk.Text(bulk, height=4, bg=C["bg3"], fg=C["text"],
                                     relief="flat", bd=0, insertbackground=C["text"])
        self._cm_bulk_text.pack(fill="x", pady=(4, 4))
        tk.Button(bulk, text="Add Bulk", font=("Courier New", 8, "bold"),
                  bg=C["accent2"], fg="#000", relief="flat", bd=0,
                  cursor="hand2", command=self._cm_bulk_add).pack(anchor="e")

        if not self._can("contact_manage"):
            for b in [self._cm_btn_add, self._cm_btn_update, self._cm_btn_delete]:
                b.configure(state="disabled", bg=C["bg3"], fg=C["muted"])

        self._cm_refresh_region_list()
        self._cm_sync_selected_region()

    def _cm_selected_region(self):
        if not hasattr(self, "_cm_region_list"):
            return ""
        sel = self._cm_region_list.curselection()
        if not sel:
            return ""
        display = self._cm_region_list.get(sel[0])
        return display.split(" | ", 1)[0].strip()

    def _cm_refresh_region_list(self):
        if not hasattr(self, "_cm_region_list"):
            return
        current = self._cm_selected_region()
        self._cm_region_list.delete(0, "end")
        q = self._cm_region_filter.get().strip().lower() if hasattr(self, "_cm_region_filter") else ""
        for region in sorted(self._loc.keys()):
            if q and q not in region.lower():
                continue
            count = len(self._contacts.get_numbers(region))
            self._cm_region_list.insert("end", f"{region} | {count} number(s)")
        if self._cm_region_list.size() and not self._cm_region_list.curselection():
            idx = 0
            for i in range(self._cm_region_list.size()):
                if self._cm_region_list.get(i).startswith(f"{current} |"):
                    idx = i
                    break
            self._cm_region_list.selection_set(idx)
        self._cm_refresh_contacts()

    def _cm_sync_selected_region(self):
        if not hasattr(self, "_cm_region_list"):
            return
        target = self._selected or ""
        if not target:
            self._cm_refresh_contacts()
            return
        for i in range(self._cm_region_list.size()):
            if self._cm_region_list.get(i).startswith(f"{target} |"):
                self._cm_region_list.selection_clear(0, "end")
                self._cm_region_list.selection_set(i)
                self._cm_region_list.see(i)
                break
        self._cm_refresh_contacts()

    def _cm_refresh_contacts(self):
        if not hasattr(self, "_cm_tree"):
            return
        region = self._cm_selected_region()
        self._cm_selected_label.set(f"Region: {region or '-'}")
        for row in self._cm_tree.get_children():
            self._cm_tree.delete(row)
        if not region:
            return
        query = self._cm_search.get().strip().lower() if hasattr(self, "_cm_search") else ""
        for row in self._contacts.list_contacts(region):
            raw = f"{row.get('name','')} {row.get('number','')}".lower()
            if query and query not in raw:
                continue
            self._cm_tree.insert(
                "",
                "end",
                values=(
                    row.get("id", ""),
    ...
    def _cm_on_contact_select(self):
        sel = self._cm_tree.selection()
        if not sel:
            return
        values = self._cm_tree.item(sel[0], "values")
        if values:
            self._cm_name.set(values[1])
            self._cm_number.set(values[2])

    def _cm_selected_contact_id(self):
        sel = self._cm_tree.selection()
        if not sel:
            return None
        vals = self._cm_tree.item(sel[0], "values")
        return vals[0] if vals else None

    def _cm_clear_form(self):
        self._cm_name.set("")
        self._cm_number.set("")

    def _cm_add(self):
        if not self._can("contact_manage"):
            messagebox.showwarning("Permission", "Your role cannot manage contacts.")
            return
        region = self._cm_selected_region()
        if not region:
            messagebox.showinfo("Contacts", "Select a region first.")
            return
        try:
            row = self._contacts.add_contact(region, self._cm_name.get(), self._cm_number.get())
            self._cm_refresh_region_list()
            self._cm_clear_form()
            self._sec_log(action="CONTACT_ADDED", status="OK", city=region,
                          detail=f"contact_id={row.get('id','-')};number={row.get('number','-')}")
        except Exception as exc:
            messagebox.showerror("Contact", str(exc))
    ...
    def _cm_update(self):
        if not self._can("contact_manage"):
            ...
            return
        region = self._cm_selected_region()
        contact_id = self._cm_selected_contact_id()
        if not region or not contact_id:
            messagebox.showinfo("Contact", "Select a contact row first.")
            return
        try:
            updated = self._contacts.update_contact(region, contact_id,
                                                    name=self._cm_name.get(),
                                                    number=self._cm_number.get())
            if not updated:
                messagebox.showwarning("Contact", "Contact not found.")
                return
            self._cm_refresh_region_list()
            self._sec_log(action="CONTACT_UPDATED", status="OK", city=region,
                          detail=f"contact_id={contact_id};number={updated.get('number','-')}")
        except Exception as exc:
            messagebox.showerror("Contact", str(exc))
    ...
    def _cm_delete(self):
        ...
        if not region or not contact_id:
            messagebox.showinfo("Contact", "Select a contact row first.")
            return
        if not messagebox.askyesno("Delete Contact", "Delete selected contact?"):
            return
        if self._contacts.delete_contact(region, contact_id):
            self._cm_refresh_region_list()
            self._cm_clear_form()
            self._sec_log(action="CONTACT_DELETED", status="WARN", city=region,
                          detail=f"contact_id={contact_id}")

    def _cm_bulk_add(self):
        if not self._can("contact_manage"):
            messagebox.showwarning("Permission", "Your role cannot manage contacts.")
            return
        region = self._cm_selected_region()
        if not region:
            messagebox.showinfo("Contacts", "Select a region first.")
            return
        text = self._cm_bulk_text.get("1.0", "end").strip()
        if not text:
            return
        added = 0
        failed = 0
        for ln in text.splitlines():
            ...
            if "," in raw:
                ...
                name, number = "Farmer", raw
            try:
                self._contacts.add_contact(region, name.strip(), number.strip())
                added += 1
            except Exception:
                failed += 1
        self._cm_refresh_region_list()
        self._cm_bulk_text.delete("1.0", "end")
        self._sec_log(action="CONTACT_BULK_ADD", status="OK" if added else "WARN", city=region,
                      detail=f"added={added};failed={failed}")
        messagebox.showinfo("Bulk Add", f"Added: {added}\nFailed: {failed}")

    def _cm_export_region_csv(self):
        region = self._cm_selected_region()
        if not region:
            messagebox.showinfo("Export", "Select a region first.")
            return
        rows = self._contacts.list_contacts(region)
        if not rows:
            messagebox.showinfo("Export", "No contacts in selected region.")
            return
        out = filedialog.asksaveasfilename(
            title="Export Region Contacts",
            defaultextension=".csv",
            initialfile=f"contacts_{region.replace(' ', '_').lower()}.csv",
            filetypes=[("CSV", "*.csv")],
        )
        if not out:
            return
        with open(out, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["id", "name", "number", "created_at", "updated_at"])
            writer.writeheader()
            for row in rows:
                writer.writerow({k: row.get(k, "") for k in ["id", "name", "number", "created_at", "updated_at"]})
        self._sec_log(action="CONTACT_EXPORT", status="OK", city=region, detail=f"file={os.path.basename(out)}")

    def _cm_import_region_csv(self):
        if not self._can("contact_manage"):
            messagebox.showwarning("Permission", "Your role cannot manage contacts.")
            return
        region = self._cm_selected_region()
        if not region:
            messagebox.showinfo("Import", "Select a region first.")
            return
        src = filedialog.askopenfilename(title="Import Region Contacts", filetypes=[("CSV", "*.csv")])
        if not src:
            return
        added = 0
        failed = 0
        try:
            with open(src, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    name = (row.get("name") or row.get("Name") or "Farmer").strip()
                    number = (row.get("number") or row.get("Number") or "").strip()
                    if not number:
                        failed += 1
                        continue
                    try:
                        self._contacts.add_contact(region, name, number)
                        added += 1
                    except Exception:
                        failed += 1
        except Exception as exc:
            ...
            return
        self._cm_refresh_region_list()
        self._sec_log(action="CONTACT_IMPORT", status="OK" if added else "WARN", city=region,
                      detail=f"file={os.path.basename(src)};added={added};failed={failed}")
        messagebox.showinfo("Import", f"Imported: {added}\nSkipped: {failed}")

    def _cm_send_test_alert(self):
        if not self._can("send_selected"):
            messagebox.showwarning("Permission", "Your role cannot send test alerts.")
            return
        region = self._cm_selected_region()
        if not region:
            messagebox.showinfo("Test Alert", "Select a region first.")
            return
        entry = next((r for r in self._alert_sys.regions if r["region"] == region), None)
        if not entry:
            messagebox.showwarning("Test Alert", "Region entry not found.")
            return
        wx = self._cache.get(region)
        if not wx:
            lat, lon = self._loc[region]
            data = fetch_forecast(region, lat, lon)
            if not data:
                messagebox.showerror("API Error", f"No weather data for {region}.")
                return
            wx = data.get("current", {})
            self._cache[region] = wx
        event = self._alert_sys.determine_event(wx) or "rain"
        msg = self._alert_sys.create_alert(entry, event, wx)
        statuses = self._dispatch_notifications(region, event, msg)
        self._sec_log(action="CONTACT_TEST_ALERT", status="OK", city=region, event=event,
                      detail=f"rows={len(statuses)}")
        messagebox.showinfo("Test Alert", f"Test alert sent for {region}.\nDeliveries logged.")
    ...
    def _dispatch_notifications(self, city, event, message):
        targets = self._contacts.get_numbers(city)
        statuses = self._notifier.send_alert(city, event, message, targets=targets)
        ...
        self._log_delivery_rows(city, event, statuses)
        return statuses

    def _log_delivery_rows(self, city, event, statuses):
        for row in statuses:
    ...
    def _auto_send(self):
        ...
        def work():
            ...
            for rgn, ev in self._events.items():
                ...
                targets = self._contacts.get_numbers(rgn)
                notify = self._notifier.send_alert(rgn, ev, msg, targets=targets)
                sent += 1
        ...
        def ok(data):
            ...
            for rgn, ev, notify_rows in sent_items:
                ...
                self._log_delivery_rows(rgn, ev, notify_rows)
    ...
    def _send_all(self):
        ...
        def work():
            ...
            for region_entry in self._alert_sys.regions:
                ...
                targets = self._contacts.get_numbers(city)
                notify = self._notifier.send_alert(city, event, msg, targets=targets)
        ...
        def ok(sent_items):
            ...
            for city, event, notify_rows in sent_items:
                ...
                self._log_delivery_rows(city, event, notify_rows)
            self._load_incidents()
